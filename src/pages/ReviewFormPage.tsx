  ...
  return <div className="form-page">
    ...
    <form className="review-form panel" onSubmit={onSubmit} noValidate>
      ...
      <fieldset><legend>1. Context</legend><div className="form-grid">
        <label><span>{t('facultySelect')}</span><select required value={draft.facultyId} onChange={(event) => update('facultyId', event.target.value)}><option value="">Select…</option>{faculty.map((person) => <option key={person.id} value={person.id}>{person.name} · {person.university}</option>)}</select></label>
