...
export function RequestFacultyPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (appMode === 'live') { form.reset(); resetTurnstile(form) }
    } catch (caught) {
  ...
  return <div className="form-page"><div className="form-intro"><span className="eyebrow">PROFILE REQUEST</span><h1>{t('requestTitle')}</h1><p>{t('requestHelp')}</p></div><form className="panel compact-form" onSubmit={submit}>
    ...
    <TurnstileWidget action="request_faculty" />{status && <p className="success-panel" role="status">{status}</p>}<button className="button primary">{t(appMode === 'demo' ? 'sendRequest' : 'sendRequestLive')}</button>
  </form></div>
}
